...
// Refresh status after preview. There's probably a better place to do this than right here.
/*if($events->have_posts()){
    foreach ($events->posts as &$event){
        $event->post_status =  get_post_status($event->ID);
    }
}*/
 ...
 * @param array $blocked
 */
$blocked_columns = apply_filters('tribe_community_events_list_columns_blocked', array('title'));
?>

<h2 class="tribe-community-events-list-title">
    <?php
    if (isset($_GET['eventDisplay']) && 'past' == $_GET['eventDisplay']) {
        echo esc_html__('My Past ' . tribe_get_event_label_plural(), 'tribe-events-community');
    } else {
        echo esc_html__('My Upcoming ' . tribe_get_event_label_plural(), 'tribe-events-community');
    } ?>
</h2>
...
<div class="tribe-nav tribe-nav-top">
    <div class="my-events-display-options ce-top">
        <?php M16_Events_Submit::tribe_community_events_prev_next_nav(); ?>
    </div>

    <div class="table-menu-wrapper ce-top">
        <?php if ($events->have_posts()) : ?>
            <a class="table-menu-btn home-btn" href="#">
    ...
    <div class="cf logout">
        <?php echo do_shortcode('[logout_to_home class="home-btn logout"]') ?>
    </div>
    ...
    <div class="tribe-responsive-table-container">
        ...
        >
            ...
            <?php while ($events->have_posts()) : ?>
                ...
                // Set post status back after monkeying with it for the preview
                $event->post_status = get_post_status($event->ID);
                ?>
